...
class APITutorialRouter(APITutorialOperator):
    ...
    def create_router(self):
        ...
        @router.patch("", tags=[])
        async def update_tutorials(update_data: list[update_schemas], where_field: str = Query(...)):
            """
            Update existing tutorials with partial data.
            
            Args:
                update_data: List of tutorial update objects with IDs and updated fields
                where_field: Field name to use in WHERE clause for updates
            ...
            try:
                ...
                updated_tutorials = await self.update(update_data, where_field)
